...
public partial class OperationViewModel : ObservableObject, IDisposable
{
    ...
    // Выполняет автосохранение при необходимости
    ...
    public async Task ScanBox(string barcode)
    {
        ...
        try
        {
            var exists = await _dbHelper.IsAnyBoxExistsByBarcode(barcode);
            if (exists)
            {
    ...
    // Устанавливает локацию для всех коробок
    [RelayCommand]
    ...
    public async Task ConfirmOperation(string? comment = null)
    {
        ...
        {
            ErrorMessage = "Нет коробок для подтверждения";
            return;
        }
    ...
    // Создает локальную коробку из штрихкода
    private async Task<Box> CreateLocalBox(string barcode)
    {
        ...
        var existsInCache = await _dbHelper.IsActiveBoxNumberExists(boxNumber);
        if (existsInCache)
        {
